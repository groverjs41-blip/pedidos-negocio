<div style="max-width: 600px; margin: 0 auto; display: flex; flex-direction: column; gap: 1.5rem;">
    <div class="page-header" style="margin-bottom: 0;">
        <div>
            ...
            <div class="page-header-subtitle">
                Ingrese los datos del cliente para la toma de pedidos y entrega.
            </div>
        </div>
    </div>
    ...
    <form wire:submit.prevent="save" class="card" style="padding: 1.5rem; display: flex; flex-direction: column; gap: 1.25rem;">
        <div style="display: flex; flex-direction: column; gap: 0.35rem;">
            ...
            <input type="text" wire:model="name" class="form-input" placeholder="Ej. Juan Pérez" required>
            @error('name') <span style="color: var(--danger-text); font-size: 0.775rem;">{{ $message }}</span> @enderror
        </div>

        <div style="display: flex; flex-direction: column; gap: 0.35rem;">
            <label class="form-label">Teléfono (Opcional)</label>
            <input type="text" wire:model="phone" class="form-input" placeholder="Ej. 809-555-0199">
            @error('phone') <span style="color: var(--danger-text); font-size: 0.775rem;">{{ $message }}</span> @enderror
        </div>

        <div style="display: flex; flex-direction: column; gap: 0.35rem;">
            <label class="form-label">Dirección (Opcional)</label>
            <textarea wire:model="address" rows="2" class="form-input" placeholder="Calle, número, sector..." style="resize: none;"></textarea>
        ...
        <div style="display: flex; flex-direction: column; gap: 0.35rem;">
            ...
            <input type="text" wire:model="addressReference" class="form-input" placeholder="Ej. Frente al parque, puerta azul">
        ...
        <div style="display: flex; flex-direction: column; gap: 0.35rem;">
            ...
            <textarea wire:model="notes" rows="2" class="form-input" placeholder="Preferencias o notas generales..." style="resize: none;"></textarea>
            @error('notes') <span style="color: var(--danger-text); font-size: 0.775rem;">{{ $message }}</span> @enderror
        </div>

        <div style="display: flex; align-items: center; gap: 0.5rem;">
            <input type="checkbox" id="customerActive" wire:model="active">
            <label for="customerActive" style="font-weight: 700; color: var(--text-main); font-size: 0.875rem;">Cliente Activo</label>
        </div>
        ...
        <div style="display: flex; justify-content: flex-end; gap: 0.85rem; margin-top: 1rem; border-top: 1px solid var(--border); padding-top: 1.25rem;">
            <a href="{{ url('/gestion/clientes') }}" class="chip-btn" style="padding: 0.75rem 1.25rem; text-decoration: none; font-weight: 700;">
                CANCELAR
            </a>
            <button
                type="submit"
